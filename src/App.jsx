...
export function App() {
  ...
  const moveExercise = (index, direction) => setStudents((items) => items.map((item) => {
    ...
    if (target < 0 || target >= item.plan.length || item.plan[target].section !== item.plan[index].section) return item;
